...
#include "ntlog.h"
	...
	/* (NT-23404). Also, it returns -1 if buffer is too small.           */
...
static int	test_vsnprintf(void)
{
	...
	nt_log(LOG_LEVEL_DEBUG, "vsnprintf() returned %d", res);
	...
	nt_log(LOG_LEVEL_CRIT, "vsnprintf() returned %d", res);
...
static wchar_t	*nt_to_unicode(unsigned int codepage, const char *cp_string)
{
