...
char	*nt_string_replace(const char *str, const char *sub_str1, const char *sub_str2)
{
	...
	if (0 == count)
		return nt_strdup(NULL, str);

	diff = (long)strlen(sub_str2) - len;

	/* allocate new memory */
	new_str = (char *)nt_malloc(new_str, (strlen(str) + (size_t)(count * diff) + 1) * sizeof(char));
...
{
	nt_rtrim(str, charlist);
	nt_ltrim(str, charlist);
}
...
char	*nt_dyn_escape_string(const char *src, const char *charlist)
{
	...
	sz = nt_get_escape_string_len(src, charlist) + 1;

	dst = (char *)nt_malloc(dst, sz);
...
{
	return nt_str_n_in_list(list, value, strlen(value), delimiter);
}
...
char	*nt_str_linefeed(const char *src, size_t maxline, const char *delim)
{
	...
	dst = (char *)nt_malloc(dst, dst_size);
...
{
	*arr = (char **)nt_malloc(*arr, sizeof(char *));
	**arr = NULL;
}
...
void	nt_strarr_add(char ***arr, const char *entry)
{
	...
	*arr = (char **)nt_realloc(*arr, sizeof(char *) * (i + 2));

	(*arr)[i] = nt_strdup((*arr)[i], entry);
	(*arr)[++i] = NULL;
}
...
{
	nt_strncpy_alloc(str, alloc_len, offset, src, strlen(src));
}

void	nt_chrcpy_alloc(char **str, size_t *alloc_len, size_t *offset, char c)
{
	nt_strncpy_alloc(str, alloc_len, offset, &c, 1);
}
...
void	nt_strquote_alloc_opt(char **str, size_t *str_alloc, size_t *str_offset, const char *value_str, int option)
{
	...
	for (size = 2, src = value_str; '\0' != *src; src++)
	{
		...
		{
			case '\\':
				if (NT_STRQUOTE_SKIP_BACKSLASH == option)
					break;
				NT_FALLTHROUGH;
			case '"':
				size++;
 ...
 *                                                                            *
 ******************************************************************************/
static void	nt_string_split(const char *src, char delimiter, unsigned char last, char **left, char **right)
{
	char	*delimiter_ptr;

	if (NULL == (delimiter_ptr = (0 == last ? strchr(src, delimiter) : strrchr(src, delimiter))))
	{
		*left = nt_strdup(NULL, src);
		*right = NULL;
	}
	else
	{
		...
		*left = nt_malloc(NULL, left_size);
		*right = nt_malloc(NULL, right_size);
...
{
	nt_string_split(src, delimiter, 0, left, right);
}

void	nt_strsplit_last(const char *src, char delimiter, char **left, char **right)
{
	nt_string_split(src, delimiter, 1, left, right);
}
 ...
 *           nt_strdcat(NULL, "") will return "", not NULL!                  *
 *                                                                            *
 ******************************************************************************/
...
#	define NT_BSUFFIX	"[...]"
	...
	if (char_max >= (key_char_total = nt_strlen_utf8(key)))
		return key;

	if (NULL != (bracket_l = strchr(key, '[')))
		is_bracket = 1;

	if (char_max < NT_CONST_STRLEN(NT_SUFFIX) + 2 * is_bracket)	/* [...] or ... */
		return key;
	...
	if (0 != is_bracket)
	{
		...
		key_char_count = nt_charcount_utf8_nbytes(key, bracket_l - key);
		param_char_count = key_char_total - key_char_count;

		if (param_char_count <= NT_CONST_STRLEN(NT_BSUFFIX))
		{
			if (char_max < param_char_count + NT_CONST_STRLEN(NT_SUFFIX))
				return key;

			key_byte_count = 1 + nt_strlen_utf8_nchars(key, char_max - param_char_count -
					NT_CONST_STRLEN(NT_SUFFIX));
			param_byte_count = 1 + nt_strlen_utf8_nchars(bracket_l, key_char_count);

			if (buf_len < key_byte_count + NT_CONST_STRLEN(NT_SUFFIX) + param_byte_count - 1)
				return key;

			key_byte_count = nt_strlcpy_utf8(buf, key, key_byte_count);
			key_byte_count += nt_strlcpy_utf8(&buf[key_byte_count], NT_SUFFIX, sizeof(NT_SUFFIX));
			nt_strlcpy_utf8(&buf[key_byte_count], bracket_l, param_byte_count);

			return buf;
		}

		if (key_char_count + NT_CONST_STRLEN(NT_BSUFFIX) > char_max)
		{
			if (char_max <= NT_CONST_STRLEN(NT_SUFFIX) + NT_CONST_STRLEN(NT_BSUFFIX))
				return key;

			key_byte_count = 1 + nt_strlen_utf8_nchars(key, char_max - NT_CONST_STRLEN(NT_SUFFIX) -
					NT_CONST_STRLEN(NT_BSUFFIX));

			if (buf_len < key_byte_count + NT_CONST_STRLEN(NT_SUFFIX) + NT_CONST_STRLEN(NT_BSUFFIX))
				return key;

			key_byte_count = nt_strlcpy_utf8(buf, key, key_byte_count);
			key_byte_count += nt_strlcpy_utf8(&buf[key_byte_count], NT_SUFFIX, sizeof(NT_SUFFIX));
			nt_strlcpy_utf8(&buf[key_byte_count], NT_BSUFFIX, sizeof(NT_BSUFFIX));

			return buf;
		}
	}

	key_byte_count = 1 + nt_strlen_utf8_nchars(key, char_max - (NT_CONST_STRLEN(NT_SUFFIX) + is_bracket));

	if (buf_len < key_byte_count + NT_CONST_STRLEN(NT_SUFFIX) + is_bracket)
		return key;

	key_byte_count = nt_strlcpy_utf8(buf, key, key_byte_count);
	nt_strlcpy_utf8(&buf[key_byte_count], NT_SUFFIX, sizeof(NT_SUFFIX));

	if (0 != is_bracket)
		nt_strlcpy_utf8(&buf[key_byte_count + NT_CONST_STRLEN(NT_SUFFIX)], "]", sizeof("]"));
...
#	define NT_SUFFIX	"..."
	...
	if (char_max >= nt_strlen_utf8(val))
		return val;

	key_byte_count = 1 + nt_strlen_utf8_nchars(val, char_max - NT_CONST_STRLEN(NT_SUFFIX));

	if (buf_len < key_byte_count + NT_CONST_STRLEN(NT_SUFFIX))
		return val;

	key_byte_count = nt_strlcpy_utf8(buf, val, key_byte_count);
	nt_strlcpy_utf8(&buf[key_byte_count], NT_SUFFIX, sizeof(NT_SUFFIX));
...
char	*nt_strcasestr(const char *haystack, const char *needle)
{
	...
	{
		if (0 == nt_strncasecmp(p, needle, sz_n))
			return (char *)p;
	}
...
static int	get_codepage(const char *encoding, unsigned int *codepage)
{
	...
	for (i = 0; 0 != cp[i].codepage || NULL != cp[i].name; i++)
	{
		nt_snprintf(buf, sizeof(buf), "cp%li", cp[i].codepage);
		if (0 == strcmp(encoding, buf))
		{
			*codepage = cp[i].codepage;
			return SUCCEED;
...
char	*nt_unicode_to_utf8(const wchar_t *wide_string)
{
	...
	utf8_string = (char *)nt_malloc(utf8_string, (size_t)utf8_size);
...
#define STATIC_SIZE	1024
	...
	if ('\0' == *encoding)
	{
		...
		out_utf8_string = nt_malloc(out_utf8_string, utf8_size);
	...
	{
		*error = nt_dsprintf(NULL, "Failed to convert from encoding %s to utf8. Failed to get codepage.",
				encoding);

		return NULL;
	}

	nt_log(LOG_LEVEL_DEBUG, "nt_convert_to_utf8() in_size:%d encoding:'%s' codepage:%u", in_size, encoding,
			codepage);
	...
	else if (1201 == codepage)	/* unicodeFFFE UTF-16, big-endian byte order */
	{
		...
		if (wide_size > STATIC_SIZE)
			wide_string = (wchar_t *)nt_malloc(wide_string, (size_t)wide_size * sizeof(wchar_t));
		else
	...
	else
	{
		...
		{
			nt_log(LOG_LEVEL_DEBUG, "nt_convert_to_utf8() Failed to convert from encoding %s"
	...
	if (0 != wide_size)
	{
		...
		out_utf8_string = (char *)nt_malloc(out_utf8_string, (size_t)utf8_size + 1/* '\0' */);
		...
		out_utf8_string = (char *)nt_calloc(out_utf8_string, 1/* '\0' */, sizeof(char));
...
char	*nt_convert_to_utf8(char *in, size_t in_size, const char *encoding, char **error)
{
	...
	out_alloc = in_size + 1;
	p = out_utf8_string = (char *)nt_malloc(out_utf8_string, out_alloc);

	/* try to guess encoding using BOM if it exists */
	if ('\0' == *encoding)
		encoding = nt_get_bom_econding(in, in_size);
	...
	while ((size_t)(-1) == iconv(cd, &in, &in_size_left, &p, &out_size_left))
	{
		...
		p = out_utf8_string = (char *)nt_realloc(out_utf8_string, out_alloc);
		p += sz;
	}
	...
	*p = '\0';
	...
	goto out;
utf8_convert_fail:
	nt_free(out_utf8_string);
	out_utf8_string = NULL;
	*error = nt_dsprintf(NULL, "Failed to convert from encoding %s to utf8. Error: %s.", encoding,
			nt_strerror(errno));
out:
	return out_utf8_string;
}
...
size_t	nt_utf8_char_len(const char *text)
{
	if (0 == (*text & 0x80))		/* ASCII */
		...
		return 4;
#if NT_MAX_BYTES_IN_UTF8_CHAR != 4
#	error "nt_utf8_char_len() is not synchronized with NT_MAX_BYTES_IN_UTF8_CHAR"
#endif
...
size_t	nt_strlen_utf8_nchars(const char *text, size_t utf8_maxlen)
{
	...
	while ('\0' != *text && 0 < utf8_maxlen && 0 != (csz = nt_utf8_char_len(text)))
	{
		next = text + csz;
		while (next > text)
		{
...
#define NT_UTF8_REPLACE_CHAR	'?'
#define NT_CTRL_REPLACE_CHAR	'.'
 ...
 *                                                                            *
 ******************************************************************************/
static void	nt_replace_invalid_utf8_impl(char *text, int replace_nonprintable)
{
	char	*out = text;
...
#undef NT_UTF8_REPLACE_CHAR
#undef NT_CTRL_REPLACE_CHAR
...
{
	nt_replace_invalid_utf8_impl(text, 0);
}
...
{
	nt_replace_invalid_utf8_impl(text, 1);
}
	...
	if (0 != sz_changed)
	{
		...
		*data_len += sz_changed;
		...
		if (*data_len > *data_alloc)
		{
			...
			*data = (char *)nt_realloc(*data, *data_alloc);
		}
...
char	*nt_dyn_escape_shell_single_quote(const char *arg)
{
	...
	pout = arg_esc = (char *)nt_malloc(NULL, len);
...
char	*nt_substr(const char *src, size_t left, size_t right)
{
	...
	str = nt_malloc(NULL, right - left + 2);
...
void	nt_replace_string(char **data, size_t l, size_t *r, const char *value)
{
	...
	if (sz_value != sz_block)
	{
		...
		if (sz_value > sz_block)
			*data = (char *)nt_realloc(*data, sz_data + 1);
