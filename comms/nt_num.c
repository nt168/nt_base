...
#include <errno.h>   /* 如果你用到了 errno / EDOM / ERANGE */
#include <math.h>    /* 这里引入 HUGE_VAL, HUGE_VALF, HUGE_VALL 等 */
 ...
 *             n     - [IN] string length or NT_MAX_UINT64_LEN               *
...
{
	nt_uint64_t		value_uint64 = 0, c;
	const nt_uint64_t	max_uint64 = ~__UINT64_C(0);

	if (NULL == str || '\0' == *str || 0 == n || sizeof(nt_uint64_t) < size || (0 == size && NULL != value))
		return FAIL;
	...
	while ('\0' != *str && 0 < n--)
	{
		...
		c = (nt_uint64_t)(unsigned char)(*str - '0');
	...
	if (NULL != value)
	{
		...
		unsigned short	value_offset = (unsigned short)((sizeof(nt_uint64_t) - size) << 8);
...
int	nt_is_hex_n_range(const char *str, size_t n, void *value, size_t size, nt_uint64_t min, nt_uint64_t max)
{
	...
	if ('\0' == *str || 0 == n || sizeof(nt_uint64_t) < size || (0 == size && NULL != value))
		return FAIL;
...
static double	NT_FLOAT_EPSILON = 0.0001;
static double	NT_DOUBLE_EPSILON = 2.22e-16;

double	nt_get_float_epsilon(void)
{
	return NT_FLOAT_EPSILON;
}

double	nt_get_double_epsilon(void)
{
	return NT_DOUBLE_EPSILON;
}

void	nt_update_epsilon_to_float_precision(void)
{
	NT_DOUBLE_EPSILON = 0.000001;
}

void	nt_update_epsilon_to_python_compatible_precision(void)
{
	NT_DOUBLE_EPSILON = 0.0001;
}

int	nt_double_compare(double a, double b)
{
	return fabs(a - b) <= NT_DOUBLE_EPSILON ? SUCCEED : FAIL;
}
...
int	nt_int_in_list(char *list, int value)
{
	...
	nt_log(LOG_LEVEL_DEBUG, "In %s() list:'%s' value:%d", __func__, list, value);
...
static int	is_double_valid_syntax(const char *str)
{
	...
	if (FAIL == nt_number_parse(str, &len))
		return FAIL;
...
int	nt_is_double(const char *str, double *value)
{
	...
	/* Not all strings accepted by strtod() can be accepted in Nt. */
...
const char	*nt_print_double(char *buffer, size_t size, double val)
{
	...
	{
		nt_snprintf(buffer, size, NT_FS_DBL64, val);
	}
	else
	{
		nt_snprintf(buffer, size, "%.15G", val);
...
int	nt_str2uint64(const char *str, const char *suffixes, nt_uint64_t *value)
{
	...
	if (SUCCEED == (ret = nt_is_uint64_n(str, sz, value)))
		*value *= factor;
 ...
 *                                                                            *
 ******************************************************************************/
static void	nt_trim_number(char *str, int strip_plus_sign)
{
...
int	nt_is_int(const char *str, int *value)
{
	...
	if (SUCCEED != nt_is_uint31(ptr, &value_ui32))
		return FAIL;
