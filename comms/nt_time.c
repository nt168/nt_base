...
{
	static NT_THREAD_LOCAL nt_timespec_t	last_ts = {0, 0};
	static NT_THREAD_LOCAL int		corr = 0;
#if defined(_WINDOWS) || defined(__MINGW32__)
	static NT_THREAD_LOCAL LARGE_INTEGER	tickPerSecond = {0};
	struct _timeb				tb;
	int					sec_diff;
#else
	struct timeval	tv;
...
#	endif
	...
	if (0 != tickPerSecond.QuadPart)
	{
		...
		{
			static NT_THREAD_LOCAL LARGE_INTEGER	last_tick = {0};
...
{
	return nt_time() + NT_JAN_1970_IN_SEC;
}
...
// 	_ftime(&current_time);
// 	*tm = *nt_localtime(&current_time.time, NULL);
// 	*milliseconds = current_time.millitm;
// #else
// 	struct timeval	current_time;

// 	gettimeofday(&current_time, NULL);
// 	*tm = *nt_localtime(&current_time.tv_sec, NULL);
...
// 		long	offset;
// #if defined(_WINDOWS) || defined(__MINGW32__)
// 		offset = nt_get_timezone_offset(current_time.time, tm);
// #else
...
// 		offset += (SUCCEED == nt_is_leap_year(tm_utc.tm_year++) ? SEC_PER_YEAR + SEC_PER_DAY : SEC_PER_YEAR);
 ...
 *                                                                            *
 ******************************************************************************/
static struct tm	*nt_localtime_r(const time_t *time)
{
	time_t					time_zerro = (time_t)0;
	static NT_THREAD_LOCAL struct tm	tm_safe;
	...
	{
		nt_log(LOG_LEVEL_DEBUG, "Wrong time value " NT_FS_TIME_T, (nt_fs_time_t)(*time));
...
struct tm	*nt_localtime(const time_t *time, const char *tz)
{
	...
	if (NULL == tz || 0 == strcmp(tz, "system"))
		return nt_localtime_r(time);

	if (NULL != (old_tz = getenv("TZ")))
		old_tz = nt_strdup(NULL, old_tz);

	setenv("TZ", tz, 1);

	tzset();

	tm = nt_localtime_r(time);
...
{
	static NT_THREAD_LOCAL struct tm	tm_last;
	static NT_THREAD_LOCAL time_t		time_last;

	if (time_last != *time)
	{
		time_last = *time;
		tm_last = *nt_localtime(time, NULL);
	}
...
#define NT_LEAP_YEARS(year)	(((year) - 1) / 4 - ((year) - 1) / 100 + ((year) - 1) / 400)
	...
	if (epoch_year <= year && 1 <= mon && mon <= 12 && 1 <= mday && mday <= nt_day_in_month(year, mon) &&
			0 <= hour && hour <= 23 && 0 <= min && min <= 59 && 0 <= sec && sec <= 61 &&
			0 <= (*t = (year - epoch_year) * SEC_PER_YEAR +
			(NT_LEAP_YEARS(2 < mon ? year + 1 : year) - NT_LEAP_YEARS(epoch_year)) * SEC_PER_DAY +
...
int	nt_day_in_month(int year, int mon)
{
	...
	if (1 <= mon && mon <= 12)	/* add one day in February of a leap year */
		return month[mon - 1] + (2 == mon && SUCCEED == nt_is_leap_year(year) ? 1 : 0);
...
nt_uint64_t	nt_get_duration_ms(const nt_timespec_t *ts)
{
	...
	return (nt_uint64_t)((now.sec - ts->sec) * 1e3 + (now.ns - ts->ns) / 1e6);
}

static void	tm_add(struct tm *tm, int multiplier, nt_time_unit_t base);
static void	tm_sub(struct tm *tm, int multiplier, nt_time_unit_t base);

static int	time_unit_seconds[NT_TIME_UNIT_COUNT] = {0, 1, SEC_PER_MIN, SEC_PER_HOUR, SEC_PER_DAY, SEC_PER_WEEK, 0,
		0, 0};
...
nt_time_unit_t	nt_tm_str_to_unit(const char *text)
{
	...
	{
		case 's':
			return NT_TIME_UNIT_SECOND;
		case 'm':
			return NT_TIME_UNIT_MINUTE;
		case 'h':
			return NT_TIME_UNIT_HOUR;
		case 'd':
			return NT_TIME_UNIT_DAY;
		case 'w':
			return NT_TIME_UNIT_WEEK;
		case 'M':
			return NT_TIME_UNIT_MONTH;
		case 'y':
			return NT_TIME_UNIT_YEAR;
		default:
			return NT_TIME_UNIT_UNKNOWN;
	}
}
...
int	nt_tm_parse_period(const char *period, size_t *len, int *multiplier, nt_time_unit_t *base, char **error)
{
	...
	if (FAIL == nt_is_uint_n_range(period, (size_t)(ptr - period), multiplier, sizeof(*multiplier), 0, UINT32_MAX))
	{
		*error = nt_strdup(*error, "invalid period multiplier");
		return FAIL;
	}

	if (NT_TIME_UNIT_UNKNOWN == (*base = nt_tm_str_to_unit(ptr)))
	{
		*error = nt_strdup(*error, "invalid period time unit");
		return FAIL;
	}
...
static void	tm_add_seconds(struct tm *tm, int seconds)
{
	...
	time_new += seconds;
	tm_new = *nt_localtime(&time_new, NULL);

	if (tm->tm_isdst != tm_new.tm_isdst && -1 != tm->tm_isdst && -1 != tm_new.tm_isdst)
	{
		if (0 == tm_new.tm_isdst)
			tm_add(&tm_new, 1, NT_TIME_UNIT_HOUR);
		else
			tm_sub(&tm_new, 1, NT_TIME_UNIT_HOUR);
	}
...
static void	tm_add(struct tm *tm, int multiplier, nt_time_unit_t base)
{
	...
	switch (base)
	{
		case NT_TIME_UNIT_HOUR:
			tm->tm_hour += multiplier;
			if (24 <= tm->tm_hour)
			{
				shift = tm->tm_hour / 24;
				tm->tm_hour %= 24;
				tm_add(tm, shift, NT_TIME_UNIT_DAY);
			}
			break;
		case NT_TIME_UNIT_DAY:
			tm->tm_mday += multiplier;
			while (tm->tm_mday > (shift = nt_day_in_month(tm->tm_year + 1900, tm->tm_mon + 1)))
			{
				tm->tm_mday -= shift;
				tm_add(tm, 1, NT_TIME_UNIT_MONTH);
			}
			tm->tm_wday += multiplier;
			tm->tm_wday %= 7;
			break;
		case NT_TIME_UNIT_WEEK:
			tm_add(tm, multiplier * 7, NT_TIME_UNIT_DAY);
			break;
		case NT_TIME_UNIT_MONTH:
			tm->tm_mon += multiplier;
			if (12 <= tm->tm_mon)
			{
				shift = tm->tm_mon / 12;
				tm->tm_mon %= 12;
				tm_add(tm, shift, NT_TIME_UNIT_YEAR);
			}
			break;
		case NT_TIME_UNIT_YEAR:
...
{
	if (NT_TIME_UNIT_MONTH == base || NT_TIME_UNIT_YEAR == base)
	{
		int	days_max;

		tm_add(tm, multiplier, base);

		days_max = nt_day_in_month(tm->tm_year + 1900, tm->tm_mon + 1);
	...
	switch (base)
	{
		case NT_TIME_UNIT_HOUR:
			...
			if (0 > tm->tm_hour)
			{
				...
				if (0 != tm->tm_hour)
					shift++;
				tm_sub(tm, shift, NT_TIME_UNIT_DAY);
			}
			return;
		case NT_TIME_UNIT_DAY:
			tm->tm_mday -= multiplier;
			while (0 >= tm->tm_mday)
			{
				...
				tm->tm_mday += nt_day_in_month(tm->tm_year + 1900, prev_mon);
				tm_sub(tm, 1, NT_TIME_UNIT_MONTH);
			}
			tm->tm_wday -= multiplier;
			if (0 > tm->tm_wday)
			...
			return;
		case NT_TIME_UNIT_WEEK:
			tm_sub(tm, multiplier * 7, NT_TIME_UNIT_DAY);
			return;
		case NT_TIME_UNIT_MONTH:
			tm->tm_mon -= multiplier;
			if (0 > tm->tm_mon)
			{
				...
				if (0 != tm->tm_mon)
					shift++;
				tm_sub(tm, shift, NT_TIME_UNIT_YEAR);
			}
			return;
		case NT_TIME_UNIT_YEAR:
			tm->tm_year -= multiplier;
...
{
	if (NT_TIME_UNIT_ISOYEAR == base)
	{
		int	week_num, total_weeks;

		week_num = nt_get_week_number(tm);

		/* use nt_tm_sub instead of tm_sub to force weekday recalculation */
		nt_tm_sub(tm, week_num, NT_TIME_UNIT_WEEK);

		total_weeks = nt_get_week_number(tm);
		if (week_num > total_weeks)
			week_num--;
		tm_sub(tm, nt_get_week_number(tm) - week_num, NT_TIME_UNIT_WEEK);
	}
	else if (NT_TIME_UNIT_MONTH == base || NT_TIME_UNIT_YEAR == base)
	{
		int	days_max;
...
void	nt_tm_round_up(struct tm *tm, nt_time_unit_t base)
{
	...
	{
		tm->tm_sec = 0;
		nt_tm_add(tm, 1, NT_TIME_UNIT_MINUTE);
	}

	if (NT_TIME_UNIT_MINUTE == base)
		return;

	if (0 != tm->tm_min)
	{
		tm->tm_min = 0;
		nt_tm_add(tm, 1, NT_TIME_UNIT_HOUR);
	}

	if (NT_TIME_UNIT_HOUR == base)
		return;

	if (0 != tm->tm_hour)
	{
		tm->tm_hour = 0;
		nt_tm_add(tm, 1, NT_TIME_UNIT_DAY);
	}

	if (NT_TIME_UNIT_DAY == base)
		return;

	if (NT_TIME_UNIT_WEEK == base)
	{
		if (1 != tm->tm_wday)
		{
			nt_tm_add(tm, (0 == tm->tm_wday ? 1 : 8 - tm->tm_wday), NT_TIME_UNIT_DAY);
			tm->tm_wday = 1;
		}
		return;
	}
	...
	{
		tm->tm_mday = 1;
		nt_tm_add(tm, 1, NT_TIME_UNIT_MONTH);
	}

	if (NT_TIME_UNIT_MONTH == base)
		return;

	if (0 != tm->tm_mon)
	{
		tm->tm_mon = 0;
		nt_tm_add(tm, 1, NT_TIME_UNIT_YEAR);
	}
...
void	nt_tm_round_down(struct tm *tm, nt_time_unit_t base)
{
	switch (base)
	{
		case NT_TIME_UNIT_WEEK:
			if (1 != tm->tm_wday)
			{
				nt_tm_sub(tm, (0 == tm->tm_wday ? 6 : tm->tm_wday - 1), NT_TIME_UNIT_DAY);
				tm->tm_wday = 1;
			}
			...
			tm->tm_sec = 0;
			break;
		case NT_TIME_UNIT_ISOYEAR:
			nt_tm_round_down(tm, NT_TIME_UNIT_WEEK);
			nt_tm_sub(tm, nt_get_week_number(tm) - 1, NT_TIME_UNIT_WEEK);
			break;
		case NT_TIME_UNIT_YEAR:
			tm->tm_mon = 0;
			NT_FALLTHROUGH;
...
{
	static NT_THREAD_LOCAL char	str[32];

	time_t		ts_time = ts->sec;
	struct tm	tm;

	tm = *nt_localtime(&ts_time, NULL);
	nt_snprintf(str, sizeof(str), "%04d.%02d.%02d %02d:%02d:%02d.%09d", tm.tm_year + 1900, tm.tm_mon + 1,
...
int	nt_get_week_number(const struct tm *tm)
{
	...
	if (0 > (days = get_week_days(tm->tm_yday, tm->tm_wday)))
	{
		...
		if (SUCCEED == nt_is_leap_year(tm->tm_year + 1899))
			d++;
	...
	else
	{
		...
		d = tm->tm_yday - 365;
		if (SUCCEED == nt_is_leap_year(tm->tm_year + 1900))
			d--;
 ...
 *                      NT_LENGTH_UNLIMITED to validate full string          *
 *                                                                            *
...
int	nt_is_time_suffix(const char *str, int *value, int length)
{
	...
	if ((NT_LENGTH_UNLIMITED == length && '\0' != *str) || (NT_LENGTH_UNLIMITED != length && 0 != len))
		return FAIL;
...
/******************************************************************************
 *                                                                            *
 * Purpose: calculate sleep time for Nt processes                         *
 *                                                                            *
...
char	*nt_age2str(time_t age)
{
	...
	if (0 != days)
		offset += nt_snprintf(buffer + offset, sizeof(buffer) - offset, "%dd ", days);
	if (0 != days || 0 != hours)
		offset += nt_snprintf(buffer + offset, sizeof(buffer) - offset, "%dh ", hours);
	if (0 != days || 0 != hours || 0 != minutes)
		offset += nt_snprintf(buffer + offset, sizeof(buffer) - offset, "%dm ", minutes);

	nt_snprintf(buffer + offset, sizeof(buffer) - offset, "%ds", seconds);
...
char	*nt_date2str(time_t date, const char *tz)
{
	...
	tm = nt_localtime(&date, tz);
	nt_snprintf(buffer, sizeof(buffer), "%.4d.%.2d.%.2d",
			tm->tm_year + 1900,
			tm->tm_mon + 1,
			tm->tm_mday);
...
char	*nt_time2str(time_t time, const char *tz)
{
	...
	tm = nt_localtime(&time, tz);
	nt_snprintf(buffer, sizeof(buffer), "%.2d:%.2d:%.2d",
			tm->tm_hour,
			tm->tm_min,
			tm->tm_sec);
	return buffer;
 ...
 *                                                                            *
 ******************************************************************************/
static int nt_iso8601_timezone(const char *zone, long int *offset)
{
	int		m, h, sign = 0;
	char		c;
	const char	*ptr = zone;
	...
	*offset = 0;
	...
	if (NT_CONST_STRLEN("0000") > strlen(ptr))
		return FAIL;

	if (SUCCEED != nt_is_uint_n_range(ptr, 2, &h, sizeof(h), 0, 23))
		return FAIL;
...
int	nt_iso8601_utc(const char *str, time_t *time)
{
	...
	if (0 == isdigit(*str) || NT_CONST_STRLEN("1234-12-12T12:12:12") > strlen(str) ||
			('T' != str[10] && ' ' != str[10]) ||
	...
	if (FAIL == nt_iso8601_timezone(&str[19], &offset))
		return FAIL;
	...
	if (NULL != time)
	{
		...
		if (FAIL == nt_utc_time(tm.tm_year, tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, &t))
			return FAIL;
...
int	nt_ts_check_deadline(const nt_timespec_t *deadline)
{
	...
	if (0 < nt_timespec_compare(&ts, deadline))
		return FAIL;
