 ...
 *                                                                            *
 ******************************************************************************/
static void	iprange_apply_mask(nt_iprange_t *iprange, int bits)
{
 ...
 *                                                                            *
 ******************************************************************************/
static int	iprangev4_parse(nt_iprange_t *iprange, const char *address)
{
	int		index, bits = -1;
	const char	*ptr = address, *dash, *end;
	size_t		len;

	iprange->type = NT_IPRANGE_V4;
	...
	{
		if (FAIL == nt_is_uint_n_range(end + 1, len - (end + 1 - address), &bits, sizeof(bits), 0, 30))
			return FAIL;
	...
	for (index = 0; ptr < end && index < NT_IPRANGE_GROUPS_V4; address = ptr + 1)
	{
		...
		if (FAIL == nt_is_uint_n_range(address, len, &iprange->range[index].from,
		...
		{
			dash++;
			if (FAIL == nt_is_uint_n_range(dash, ptr - dash, &iprange->range[index].to,
 ...
 *                                                                            *
 ******************************************************************************/
static int	iprangev6_parse(nt_iprange_t *iprange, const char *address)
{
	int		index, fill = -1, bits = -1, target;
	const char	*ptr = address, *dash, *end;
	size_t		len;

	iprange->type = NT_IPRANGE_V6;
	...
	if (NULL != (end = strchr(address, '/')))
	{
		if (FAIL == nt_is_uint_n_range(end + 1, len - (end + 1 - address), &bits, sizeof(bits), 0, 128))
			return FAIL;
	...
	/* iterate through address numbers (bit groups) */
	for (index = 0; ptr < end && index < NT_IPRANGE_GROUPS_V6; address = ptr + 1)
	{
		if (NULL == (ptr = strchr(address, ':')))
			ptr = end;
		...
		/* extract the range start value */
		if (FAIL == nt_is_hex_n_range(address, len, &iprange->range[index].from, 4, 0, (1 << 16) - 1))
			return FAIL;
		...
		{
			dash++;
			if (FAIL == nt_is_hex_n_range(dash, ptr - dash, &iprange->range[index].to, 4, 0, (1 << 16) - 1))
				return FAIL;
...
{
	if (NT_IPRANGE_V6 == type)
	{
		nt_snprintf(ip, len, "%x:%x:%x:%x:%x:%x:%x:%x", (unsigned int)ipaddress[0],
	...
	{
		nt_snprintf(ip, len, "%u.%u.%u.%u", (unsigned int)ipaddress[0], (unsigned int)ipaddress[1],
...
void	nt_iprange_first(const nt_iprange_t *iprange, int *address)
{
	...
	groups = (NT_IPRANGE_V4 == iprange->type ? NT_IPRANGE_GROUPS_V4 : NT_IPRANGE_GROUPS_V6);

	for (i = 0; i < groups; i++)
		address[i] = iprange->range[i].from;

	/* exclude network address if the IPv4 range was specified with network mask */
	if (NT_IPRANGE_V4 == iprange->type && 0 != iprange->mask)
		address[groups - 1]++;
}
...
{
	static NT_THREAD_LOCAL int	idx, ipaddress[8];

	if ('\0' == *ip)
	{
		idx = 0;
		nt_iprange_first(&ipranges[idx], ipaddress);
		nt_iprange_ip2str(ipranges[idx].type, ipaddress, ip, len);

		return SUCCEED;
	}

	if (FAIL == nt_iprange_uniq_iter(ipranges, num, &idx, ipaddress))
		return FAIL;
...
{
	int	i, z[NT_IPRANGE_GROUPS_V6] = {0};

	if (0 == num)
		return FAIL;

	if (0 == memcmp(ipaddress, z, sizeof(int) * (NT_IPRANGE_V4 == ipranges->type ?
			NT_IPRANGE_GROUPS_V4 : NT_IPRANGE_GROUPS_V6)))
	{
		*idx = 0;
		nt_iprange_first(ipranges, ipaddress);
		return SUCCEED;
	}
	...
	do
	{
		if (FAIL == nt_iprange_next(&ipranges[*idx], ipaddress))
		{
			if (++(*idx) == num)
				return FAIL;
		...
		for (i = 0; i < *idx; i++)
		{
			if (SUCCEED == nt_iprange_validate(&ipranges[i], ipaddress))
				break;
		}
	}
 ...
 *               NT_MAX_UINT64 if this number exceeds 64 bit unsigned        *
 *               integer.                                                     *
 *                                                                            *
 ******************************************************************************/
nt_uint64_t	nt_iprange_volume(const nt_iprange_t *iprange)
{
	...
	for (i = 0; i < groups; i++)
	{
		...
		if (NT_MAX_UINT64 / n < volume)
			return NT_MAX_UINT64;
...
{
	static NT_THREAD_LOCAL int	idx, current_port;

	if (0 == num)
		return FAIL;

	if (NT_PORTRANGE_INIT_PORT == *port)
	{
		idx = 0;
	...
	if (FAIL == nt_portrange_uniq_iter(ranges, num, &idx, &current_port))
		return FAIL;
