...
nt_config_tls_t	*nt_config_tls_new(void)
{
	...
	config_tls = (nt_config_tls_t *)nt_malloc(NULL, sizeof(nt_config_tls_t));

	config_tls->connect_mode	= NT_TCP_SEC_UNENCRYPTED;
	config_tls->accept_modes	= NT_TCP_SEC_UNENCRYPTED;
	...
	config_tls->frontend_accept_modes	= NT_TCP_SEC_UNENCRYPTED;
...
{
	nt_free(config_tls->connect);
	nt_free(config_tls->accept);
	nt_free(config_tls->ca_file);
	nt_free(config_tls->crl_file);
	nt_free(config_tls->server_cert_issuer);
	nt_free(config_tls->server_cert_subject);
	nt_free(config_tls->cert_file);
	nt_free(config_tls->key_file);
	nt_free(config_tls->psk_identity);
	nt_free(config_tls->psk_file);
	nt_free(config_tls->cipher_cert13);
	nt_free(config_tls->cipher_cert);
	nt_free(config_tls->cipher_psk13);
	nt_free(config_tls->cipher_psk);
	nt_free(config_tls->cipher_all13);
	nt_free(config_tls->cipher_all);
	nt_free(config_tls->cipher_cmd13);
	nt_free(config_tls->cipher_cmd);
	nt_free(config_tls->frontend_accept);
	nt_free(config_tls->tls_listen);
	nt_free(config_tls->frontend_cert_issuer);
	nt_free(config_tls->frontend_cert_subject);
...
static NT_THREAD_LOCAL char	nt_socket_strerror_message[NT_SOCKET_STRERROR_LEN];

const char	*nt_socket_strerror(void)
{
	nt_socket_strerror_message[NT_SOCKET_STRERROR_LEN - 1] = '\0';	/* force null termination */
	return nt_socket_strerror_message;
}

__nt_attr_format_printf(1, 2)
static void	nt_set_socket_strerror(const char *fmt, ...)
{
	va_list args;
...
static int	nt_socket_peer_ip_save(nt_socket_t *s)
{
	...
	{
		error_message = nt_strerror_from_system(nt_socket_last_error());
...
#ifdef HAVE_IPV6
	if (0 != nt_getnameinfo((struct sockaddr *)&sa, s->peer, sizeof(s->peer), NULL, 0, NI_NUMERICHOST))
	{
		error_message = nt_strerror_from_system(nt_socket_last_error());
		nt_set_socket_strerror("connection rejected, getnameinfo() failed: %s", error_message);
		return FAIL;
	}
#else
	nt_strscpy(s->peer, inet_ntoa(sa.sin_addr));
#endif
	return SUCCEED;
}
...
int	nt_inet_pton(int af, const char *src, void *dst)
{
	if (AF_INET == af)
	{
		...
		{
			nt_log(LOG_LEVEL_DEBUG, "cannot get ip from IPv4 address: %s", nt_strerror(errno));
			return FAIL;
		}
	...
	else if (AF_INET6 == af)
	{
		...
		{
			nt_log(LOG_LEVEL_DEBUG, "cannot get ip from IPv6 address: %s", nt_strerror(errno));
			return FAIL;
		}
	...
	nt_log(LOG_LEVEL_DEBUG, "Fail to parse IP: '%s'", src);
...
int	nt_inet_ntop(struct sockaddr *ai_addr, char *ip, socklen_t len)
{
	...
	nt_log(LOG_LEVEL_DEBUG, "unknown address family:%d", ai_addr->sa_family);
	return FAIL;
}
...
void	nt_getip_by_host(const char *host, char *ip, size_t iplen)
{
	...
	if (FAIL == nt_inet_ntop(ai->ai_addr, ip, (socklen_t)iplen))
		ip[0] = '\0';
out:
	if (NULL != ai)
		freeaddrinfo(ai);
 ...
 *                                                                            *
 ******************************************************************************/
static int nt_is_win_ver_or_greater(nt_uint32_t major, nt_uint32_t minor, nt_uint32_t servpack)
{
...
int	nt_socket_start(char **error)
{
	...
	{
		*error = nt_dsprintf(*error, "Cannot initialize Winsock DLL: %s", nt_strerror_from_system(ret));
		return FAIL;
	}
...
char 	*socket_poll_error(short revents)
{
	...
	nt_strcpy_alloc(&str, &str_alloc, &str_offset, "connection error ");

	if (0 != (revents & POLLERR))
	{
		nt_snprintf_alloc(&str, &str_alloc, &str_offset, "%c%s", delim, "POLLERR");
		delim = ',';
	}

	if (0 != (revents & POLLHUP))
	{
		nt_snprintf_alloc(&str, &str_alloc, &str_offset, "%c%s", delim, "POLLHUP");
		delim = ',';
	}

	if (0 != (revents & POLLNVAL))
		nt_snprintf_alloc(&str, &str_alloc, &str_offset, "%c%s", delim, "POLLNVAL");

	nt_chrcpy_alloc(&str, &str_alloc, &str_offset, ')');
...
int	nt_socket_pollout(nt_socket_t *s, int timeout, char **error)
{
	...
	while (0 >= (rc = nt_socket_poll(&pd, 1, timeout)))
	{
		if (-1 == rc && SUCCEED != nt_socket_had_nonblocking_error())
		{
			if (NULL != error)
				*error = nt_strdup(NULL, "cannot wait for connection");

			return FAIL;
		}

		if (SUCCEED != nt_socket_check_deadline(s))
		{
			if (NULL != error)
				*error = nt_strdup(NULL, "connection timed out");
	...
	if (POLLOUT != (pd.revents & (POLLOUT | POLLERR | POLLHUP | POLLNVAL)))
	{
		if (NULL != error)
		{
			*error = socket_poll_error(pd.revents);
			nt_log(LOG_LEVEL_DEBUG, "poll(POLLOUT) failed with revents 0x%x", (unsigned)pd.revents);
		}
		...
		int timeout)
{
...
#ifdef HAVE_IPV6
	else if (SUCCEED == nt_is_ip6(ip))
		flags = AI_NUMERICHOST;
#endif
	else
		flags = 0;

	nt_snprintf(service, sizeof(service), "%hu", port);
	nt_tcp_init_hints(&hints, type, flags);
	...
	if (NT_SOCKET_ERROR == (s->socket = socket(ai->ai_family, ai->ai_socktype | SOCK_CLOEXEC, ai->ai_protocol)))
	{
		nt_set_socket_strerror("cannot create socket [[%s]:%hu]: %s",
				ip, port, nt_strerror_from_system(nt_socket_last_error()));
		goto out;
	}
	...
	{
		nt_set_socket_strerror("failed to set the FD_CLOEXEC file descriptor flag on socket [[%s]:%hu]: %s",
				ip, port, nt_strerror_from_system(nt_socket_last_error()));
	}
#endif
	...
	if (NULL != source_ip)
	{
		nt_tcp_init_hints(&hints, type, AI_NUMERICHOST);
		...
		if (NT_PROTO_ERROR == nt_bind(s->socket, ai_bind->ai_addr, ai_bind->ai_addrlen))
		{
			nt_set_socket_strerror("bind() failed: %s", nt_strerror_from_system(nt_socket_last_error()));
			func_socket_close(s);
			goto out;
		}
	}
	...
	{
		nt_set_socket_strerror("setting non-blocking mode for [[%s]:%hu] failed: %s",
				NULL != ip ? ip : "-", port,
				nt_strerror_from_system(nt_socket_last_error()));
		func_socket_close(s);
		goto out;
	}

	if (NT_PROTO_ERROR == connect(s->socket, ai->ai_addr, ai->ai_addrlen) &&
			SUCCEED != nt_socket_had_nonblocking_error())
	{
		nt_set_socket_strerror("cannot connect to address: %s",
				nt_strerror_from_system(nt_socket_last_error()));
		func_socket_close(s);
		goto out;
	}
	...
	nt_socket_set_deadline(s, timeout);
...
{
#if defined(HAVE_GNUTLS) || defined(HAVE_OPENSSL)
	if (NT_TCP_SEC_TLS_PSK == tls_connect && '\0' == *tls_arg1)
	{
		*error = nt_strdup(*error, "cannot connect with PSK: PSK not available");
		return FAIL;
	}

	return nt_tls_connect(s, tls_connect, tls_arg1, tls_arg2, server_name, event, error);
#else
	NT_UNUSED(s);
	NT_UNUSED(tls_connect);
	NT_UNUSED(tls_arg1);
	NT_UNUSED(tls_arg2);
	NT_UNUSED(server_name);
	NT_UNUSED(event);
	NT_UNUSED(tls_connect);

	*error = nt_strdup(*error, "support for TLS was not compiled in");
	return FAIL;
#endif
}
 ...
 *                                                                            *
 ******************************************************************************/
static int	nt_socket_create(nt_socket_t *s, int type, const char *source_ip, const char *ip,
...
#endif
	if (SUCCEED != nt_socket_connect(s, type, source_ip, ip, port, timeout))
		goto out;

	if (SUCCEED != nt_socket_pollout(s, NT_SOCKET_POLL_TIMEOUT, &error))
	{
		void		(*func_socket_close)(nt_socket_t *s);
...
#if defined(HAVE_GNUTLS) || defined(HAVE_OPENSSL)
	if (SUCCEED != nt_is_ip(ip))
		server_name = ip;

	if (NT_TCP_SEC_TLS_CERT == tls_connect || NT_TCP_SEC_TLS_PSK == tls_connect)
	{
		if (SUCCEED != nt_socket_tls_connect(s, tls_connect, tls_arg1, tls_arg2, server_name, NULL, &error))
		{
			nt_tcp_close(s);
			nt_set_socket_strerror("TCP successful, cannot establish TLS to [[%s]:%hu]: %s", ip, port,
					error);
			nt_free(error);
			goto out;
		}
...
ssize_t	nt_tcp_write(nt_socket_t *s, const char *buf, size_t len, short *event)
{
	...
	if (NULL != s->tls_ctx)	/* TLS connection */
	{
		...
		if (NT_PROTO_ERROR == (n = nt_tls_write(s, buf, len, event, &error)))
		{
			nt_set_socket_strerror("%s", error);
			nt_free(error);
		}
	...
	if (0 < (n = NT_TCP_WRITE(s->socket, buf, len)) && (size_t)n == len)
		return n;
	...
	while (1)
	{
		if (0 > n)
		{
			...
			if (SUCCEED != nt_socket_had_nonblocking_error())
			{
				nt_set_socket_strerror("cannot write data: %s",
						nt_strerror_from_system(nt_socket_last_error()));
			...
			if (-1 == (rc = nt_socket_poll(&pd, 1, NT_SOCKET_POLL_TIMEOUT)))
			{
				if (SUCCEED != nt_socket_had_nonblocking_error())
				{
					nt_set_socket_strerror("cannot wait for socket: %s",
							nt_strerror_from_system(nt_socket_last_error()));
					return NT_PROTO_ERROR;
				}
			}
			else if (0 != rc && 0 == (pd.revents & POLLOUT))
			{
				...
				nt_set_socket_strerror("%s", errmsg);
				nt_free(errmsg);

				nt_log(LOG_LEVEL_DEBUG, "poll(POLLOUT) failed with revents 0x%x",
						(unsigned)pd.revents);
		...
		if (SUCCEED != nt_socket_check_deadline(s))
		{
			nt_set_socket_strerror("write timeout");
			return NT_PROTO_ERROR;
		}

		n = NT_TCP_WRITE(s->socket, buf + offset, (len - (size_t)offset));
	}
		...
		nt_tcp_send_context_t *context)
{
	...
	if (NT_MAX_RECV_LARGE_DATA_SIZE < len)
	{
		nt_set_socket_strerror("cannot send data: message size " NT_FS_UI64 " exceeds the maximum"
				" size " NT_FS_UI64 " bytes.", (nt_uint64_t)len,
				(nt_uint64_t)NT_MAX_RECV_LARGE_DATA_SIZE);
		return FAIL;
	}

	if (NT_MAX_RECV_LARGE_DATA_SIZE < reserved)
	{
		nt_set_socket_strerror("cannot send data: uncompressed message size " NT_FS_UI64
				" exceeds the maximum size " NT_FS_UI64 " bytes.", (nt_uint64_t)reserved,
				(nt_uint64_t)NT_MAX_RECV_LARGE_DATA_SIZE);
		return FAIL;
	}

	if (0 != (flags & NT_TCP_COMPRESS))
	{
		/* compress if not compressed yet */
		if (0 == reserved)
		{
			if (SUCCEED != nt_compress(data, len, &context->compressed_data, &context->send_len))
			{
				nt_set_socket_strerror("cannot compress data: %s", nt_compress_strerror());
	...
	memcpy(context->header_buf, NT_TCP_HEADER_DATA, NT_CONST_STRLEN(NT_TCP_HEADER_DATA));
	context->header_len = NT_CONST_STRLEN(NT_TCP_HEADER_DATA);
	...
	if (0 != (flags & NT_TCP_LARGE))
	{
		nt_uint64_t	len64_le;

		len64_le = nt_htole_uint64((nt_uint64_t)context->send_len);
		memcpy(context->header_buf + context->header_len, &len64_le, sizeof(len64_le));
		context->header_len += sizeof(len64_le);

		len64_le = nt_htole_uint64((nt_uint64_t)reserved);
		memcpy(context->header_buf + context->header_len, &len64_le, sizeof(len64_le));
		context->header_len += sizeof(len64_le);
	}
	else
	{
		nt_uint32_t	len32_le;

		len32_le = nt_htole_uint32((nt_uint32_t)context->send_len);
		memcpy(context->header_buf + context->header_len, &len32_le, sizeof(len32_le));
		context->header_len += sizeof(len32_le);

		len32_le = nt_htole_uint32((nt_uint32_t)reserved);
		memcpy(context->header_buf + context->header_len, &len32_le, sizeof(len32_le));
		context->header_len += sizeof(len32_le);
	}
...
#define NT_TLS_MAX_REC_LEN	16384
	...
	{
		ssize_t	data_len;
		char	buf[NT_TLS_MAX_REC_LEN];	/* Buffer is allocated on stack with a hope that it   */
		...
		data_len = MIN((ssize_t)context->send_len, NT_TLS_MAX_REC_LEN - remaining_header_len);
		memcpy(buf + remaining_header_len, context->data, (size_t)data_len);

		send_bytes = remaining_header_len + data_len;

		if (NT_PROTO_ERROR == (bytes_sent = nt_tcp_write(s, buf, (size_t)send_bytes, event)))
			return FAIL;
	...
	while (context->written < (ssize_t)context->send_len)
	{
		if (NT_TCP_SEC_UNENCRYPTED == s->connection_type)
			...
			send_bytes = MIN(NT_TLS_MAX_REC_LEN, (ssize_t)context->send_len - context->written);

		if (NT_PROTO_ERROR == (bytes_sent = nt_tcp_write(s, context->data + context->written,
		...
		int timeout)
{
	...
	if (SUCCEED == (ret = nt_tcp_send_context_init(data, len, reserved, flags, &context)))
	{
		ret = nt_tcp_send_context(s, &context, NULL);
		nt_tcp_send_context_clear(&context);
	}

	if (0 != timeout)
		nt_socket_set_deadline(s, 0);
...
int	get_address_family(const char *addr, int *family, char *error, int max_error_len)
{
	...
	{
		nt_snprintf(error, max_error_len, "%s: [%d] %s", addr, err, gai_strerror(err));
		goto out;
	}

	if (PF_INET != ai->ai_family && PF_INET6 != ai->ai_family)
	{
		nt_snprintf(error, max_error_len, "%s: unsupported address family", addr);
		goto out;
	}
...
{
#if defined(_WINDOWS)
		nt_set_socket_strerror("getaddrinfo() failed for '%s': %s",
				ip, nt_strerror_from_system(WSAGetLastError()));
#else
#if defined(HAVE_HSTRERROR)
		nt_set_socket_strerror("getaddrinfo() failed for '%s': [%d] %s",
				ip, h_errno, hstrerror(h_errno));
#else
		nt_set_socket_strerror("getaddrinfo() failed for '%s': [%d]",
				ip, h_errno);
#endif
#endif
}
 ...
 *                                                                            *
 ******************************************************************************/
static ssize_t	tcp_peek(nt_socket_t *s, char *buffer, size_t size)
{
	ssize_t		n;
	nt_pollfd_t	pd;

	if (0 <= (n = NT_TCP_RECV(s->socket, buffer, size, MSG_PEEK)))
		return n;
...
static ssize_t	tcp_read(nt_socket_t *s, char *buffer, size_t size, short *events)
{
	...
	if (0 <= (n = NT_TCP_READ(s->socket, buffer, size)))
		return n;

	if (SUCCEED != nt_socket_had_nonblocking_error())
	{
		nt_set_socket_strerror("cannot read from socket: %s",
				nt_strerror_from_system(nt_socket_last_error()));
		return NT_PROTO_ERROR;
	}
	...
	while (1)
	{
		...
		if (SUCCEED != nt_socket_check_deadline(s))
		{
			nt_set_socket_strerror("read timeout");
			return NT_PROTO_ERROR;
		}
		...
		if (0 == (pd.revents & POLLIN))
		{
			...
			nt_log(LOG_LEVEL_DEBUG, "poll(POLLIN) failed with revents 0x%x", (unsigned)pd.revents);
...
{
#if defined(_WINDOWS)
	return WSAEADDRINUSE == nt_socket_last_error() ? SUCCEED : FAIL;
#else
	return EADDRINUSE == nt_socket_last_error() ? SUCCEED : FAIL;
#endif
}
		...
		int config_tcp_max_backlog_size)
{
	...
	static NT_THREAD_LOCAL int	no_inherit_wsapi = -1;

	if (-1 == no_inherit_wsapi)
	{
		/* Both Windows 7 and Windows 2008 R2 are 0x0601 */
		no_inherit_wsapi = nt_is_win_ver_or_greater((_WIN32_WINNT_WIN7 >> 8) & 0xff,
	...
	nt_tcp_init_hints(&hints, SOCK_STREAM, AI_NUMERICHOST | AI_PASSIVE);
	nt_snprintf(port, sizeof(port), "%hu", listen_port);
	...
	while (1)
	{
		...
		{
			nt_set_socket_strerror("cannot resolve address [[%s]:%s]: [%d] %s",
		...
		for (current_ai = ai; NULL != current_ai; current_ai = current_ai->ai_next)
		{
			...
			{
				nt_set_socket_strerror("not enough space for socket [[%s]:%s]",
						NULL != ip ? ip : "-", port);
				goto out;
			}
			...
			s->sockets[s->num_socks] = WSASocket(current_ai->ai_family, current_ai->ai_socktype,
					...
					WSA_FLAG_OVERLAPPED);
			if (NT_SOCKET_ERROR == s->sockets[s->num_socks])
			{
				nt_set_socket_strerror("WSASocket() for [[%s]:%s] failed: %s",
						NULL != ip ? ip : "-", port,
						nt_strerror_from_system(nt_socket_last_error()));

				if (WSAEAFNOSUPPORT == nt_socket_last_error())
					continue;
			...
			{
				nt_log(LOG_LEVEL_WARNING, "SetHandleInformation() failed: %s",
						nt_strerror_from_system(GetLastError()));
			}
			...
			if (NT_PROTO_ERROR == setsockopt(s->sockets[s->num_socks], SOL_SOCKET, SO_EXCLUSIVEADDRUSE,
					(void *)&on, sizeof(on)))
			{
				nt_set_socket_strerror("setsockopt() with %s for [[%s]:%s] failed: %s",
			...
			{
				nt_set_socket_strerror("socket() for [[%s]:%s] failed: %s",
						NULL != ip ? ip : "-", port,
						nt_strerror_from_system(nt_socket_last_error()));

				if (EAFNOSUPPORT == nt_socket_last_error())
					continue;
...
#	if !SOCK_CLOEXEC
			...
			{
				nt_set_socket_strerror("failed to set the FD_CLOEXEC file descriptor flag on "
...
#	endif
			...
			if (PF_INET6 == current_ai->ai_family &&
					NT_PROTO_ERROR == setsockopt(s->sockets[s->num_socks], IPPROTO_IPV6,
...
#endif
			if (NT_PROTO_ERROR == nt_bind(s->sockets[s->num_socks], current_ai->ai_addr,
								current_ai->ai_addrlen))
			{
				nt_set_socket_strerror("bind() for [[%s]:%s] failed: %s",
						NULL != ip ? ip : "-", port,
						nt_strerror_from_system(nt_socket_last_error()));
			...
			if (NT_PROTO_ERROR == listen(s->sockets[s->num_socks], config_tcp_max_backlog_size))
			{
				nt_set_socket_strerror("listen() for [[%s]:%s] failed: %s",
						NULL != ip ? ip : "-", port,
						nt_strerror_from_system(nt_socket_last_error()));
				nt_socket_close(s->sockets[s->num_socks]);
				goto out;
			...
			{
				nt_set_socket_strerror("setting non-blocking mode for [[%s]:%s] failed: %s",
						NULL != ip ? ip : "-", port,
						nt_strerror_from_system(nt_socket_last_error()));
				nt_socket_close(s->sockets[s->num_socks]);
				goto out;
	...
	if (0 == s->num_socks)
	{
		nt_set_socket_strerror("nt_tcp_listen(): "
		...
		const char *unencrypted_allowed_ip)
{
	...
	pds = (nt_pollfd_t *)nt_malloc(NULL, sizeof(nt_pollfd_t) * (size_t)s->num_socks);
	...
	if (NT_PROTO_ERROR == (ret = nt_socket_poll(pds, (unsigned long)s->num_socks, poll_timeout * 1000)))
	{
		if (SUCCEED == nt_socket_had_nonblocking_error())
			ret = TIMEOUT_ERROR;
		else
			nt_set_socket_strerror("poll() failed: %s", nt_strerror_from_system(nt_socket_last_error()));
	...
	if (i == s->num_socks)
	{
		nt_set_socket_strerror("incoming connection has failed");
		goto out;
	}
			...
			&nlen)))
	{
		...
		{
			nt_set_socket_strerror("accept() failed: %s",
					nt_strerror_from_system(nt_socket_last_error()));
		}
	...
	{
		nt_set_socket_strerror("failed to set socket non-blocking mode: %s",
				nt_strerror_from_system(nt_socket_last_error()));
		nt_tcp_unaccept(s);
		goto out;
	}
	...
	nt_socket_set_deadline(s, s->timeout);
	...
	{
#if defined(HAVE_GNUTLS) || defined(HAVE_OPENSSL)
		if (0 != (tls_accept & (NT_TCP_SEC_TLS_CERT | NT_TCP_SEC_TLS_PSK)))
		{
			char	*error = NULL;

			if (SUCCEED != nt_tls_accept(s, tls_accept, &error))
			{
				nt_set_socket_strerror("from %s: %s", s->peer, error);
				nt_tcp_unaccept(s);
				nt_free(error);
				goto out;
			}
		...
		{
			nt_set_socket_strerror("from %s: TLS connections are not allowed", s->peer);
			nt_tcp_unaccept(s);
			goto out;
		}
#else
		nt_set_socket_strerror("from %s: support for TLS was not compiled in", s->peer);
		nt_tcp_unaccept(s);
		goto out;
#endif
	}
	...
	if (NT_TCP_SEC_UNENCRYPTED == s->connection_type && NULL != tls_listen)
	{
		if (NULL == unencrypted_allowed_ip || SUCCEED != nt_tcp_check_allowed_peers(s, unencrypted_allowed_ip))
		{
			nt_set_socket_strerror("from %s: unencrypted connections are not allowed", s->peer);
			nt_tcp_unaccept(s);
			goto out;
		...
		else
		{
			...
			s->max_len_limit = NT_MAX_RECV_2KB_DATA_SIZE;
		}
	}
...
#define NT_TCP_LINE_LEN	(64 * NT_KIBIBYTE)
	...
	/* Because we are reading data in NT_STAT_BUF_LEN chunks the leftover       */
	...
	if (NT_PROTO_ERROR == (nbytes = tcp_read(s, s->buf_stat + left, NT_STAT_BUF_LEN - left - 1, NULL)))
		goto out;
	...
	/* Lines larger than NT_TCP_LINE_LEN bytes will be truncated. */
	do
	{
		if (NT_PROTO_ERROR == (nbytes = tcp_read(s, buffer, NT_STAT_BUF_LEN - 1, NULL)))
			goto out;
		...
		if (s->read_bytes + (size_t)nbytes < NT_TCP_LINE_LEN && s->read_bytes == line_length)
		{
			nt_strncpy_alloc(&s->buffer, &alloc, &offset, buffer, (size_t)nbytes);
			s->read_bytes += (size_t)nbytes;
		}
		else
		{
			if (0 != (left = (NULL == ptr ? NT_TCP_LINE_LEN - s->read_bytes :
					MIN(NT_TCP_LINE_LEN - s->read_bytes, (size_t)(ptr - buffer)))))
			{
			...
			if (NULL != ptr)
			{
				nt_strncpy_alloc(&s->buffer, &alloc, &offset, ptr, (size_t)(nbytes - (ptr - buffer)));
...
ssize_t	nt_tcp_read(nt_socket_t *s, char *buf, size_t len, short *events)
{
	...
	if (NULL != s->tls_ctx)	/* TLS connection */
	{
		...
		if (NT_PROTO_ERROR == (res = nt_tls_read(s, buf, len, events, &error)))
		{
...
{
	int	ret;
	char	buf[NT_STAT_BUF_LEN];

	if (SUCCEED != nt_tls_used(s))
		return 0;
	...
	ret = nt_tcp_read(s, buf, sizeof(buf), events);
...
void	nt_socket_set_deadline(nt_socket_t *s, int timeout)
{
	...
	nt_ts_get_deadline(&s->deadline, timeout);
}
...
int	nt_socket_check_deadline(nt_socket_t *s)
{
	...
	return nt_ts_check_deadline(&s->deadline);
}

#define NT_TCP_EXPECT_HEADER		1
#define NT_TCP_EXPECT_VERSION		2
#define NT_TCP_EXPECT_VERSION_VALIDATE	3
#define NT_TCP_EXPECT_LENGTH		4
#define NT_TCP_EXPECT_SIZE		5
...
void	nt_tcp_recv_context_init(nt_socket_t *s, nt_tcp_recv_context_t *tcp_recv_context, unsigned char flags)
{
	...
	tcp_recv_context->expected_len = 16 * NT_MEBIBYTE;
	tcp_recv_context->reserved = 0;
	tcp_recv_context->expect = NT_TCP_EXPECT_HEADER;
#if defined(_WINDOWS)
	tcp_recv_context->max_len = NT_MAX_RECV_DATA_SIZE;
#else
	if (0 != s->max_len_limit)
	{
		tcp_recv_context->max_len = s->max_len_limit;
	}
	else if (0 != (flags & NT_TCP_LARGE))
	{
		tcp_recv_context->max_len = NT_MAX_RECV_LARGE_DATA_SIZE;
	}
	else
	{
		tcp_recv_context->max_len = NT_MAX_RECV_DATA_SIZE;
...
ssize_t	nt_tcp_recv_context(nt_socket_t *s, nt_tcp_recv_context_t *context, unsigned char flags, short *events)
{
	...
	while (0 != (nbytes = nt_tcp_read(s, s->buf_stat + context->buf_stat_bytes,
			sizeof(s->buf_stat) - context->buf_stat_bytes, events)))
	{
		...
		if (NT_TCP_EXPECT_HEADER == context->expect)
		{
			if (NT_TCP_HEADER_LEN > context->buf_stat_bytes)
			{
				if (0 == strncmp(s->buf_stat, NT_TCP_HEADER_DATA, context->buf_stat_bytes))
					continue;
			...
			{
				if (0 != strncmp(s->buf_stat, NT_TCP_HEADER_DATA, NT_TCP_HEADER_LEN))
				{
					/* invalid header, abort receiving */
					break;
				}

				context->expect = NT_TCP_EXPECT_VERSION;
				context->offset += NT_TCP_HEADER_LEN;
			}
		}

		if (NT_TCP_EXPECT_VERSION == context->expect)
		{
			if (context->offset + 1 > context->buf_stat_bytes)
				continue;

			context->expect = NT_TCP_EXPECT_VERSION_VALIDATE;
			context->protocol_version = s->buf_stat[NT_TCP_HEADER_LEN];

			if (0 == (context->protocol_version & NT_TCP_PROTOCOL) ||
					context->protocol_version > (NT_TCP_PROTOCOL | NT_TCP_COMPRESS | flags))
			{
				/* invalid protocol version, abort receiving */
				break;
			}
			s->protocol = context->protocol_version;
			context->expect = NT_TCP_EXPECT_LENGTH;
			context->offset++;
		}

		if (NT_TCP_EXPECT_LENGTH == context->expect)
		{
			if (0 != (context->protocol_version & NT_TCP_LARGE))
			{
				nt_uint64_t	len64_le;
				...
				context->expected_len = nt_letoh_uint64(len64_le);

				memcpy(&len64_le, s->buf_stat + context->offset, sizeof(len64_le));
				context->offset += sizeof(len64_le);
				context->reserved = nt_letoh_uint64(len64_le);
			}
			else
			{
				nt_uint32_t	len32_le;
				...
				context->expected_len = nt_letoh_uint32(len32_le);

				memcpy(&len32_le, s->buf_stat + context->offset, sizeof(len32_le));
				context->offset += sizeof(len32_le);
				context->reserved = nt_letoh_uint32(len32_le);
			}

			if (context->max_len < context->expected_len)
			{
				nt_set_socket_strerror("message exceeds the maximum size");
				nt_log(LOG_LEVEL_WARNING, "Message size " NT_FS_UI64 " from %s exceeds the "
						"maximum size " NT_FS_UI64 " bytes. Message ignored.",
			...
			{
				nt_set_socket_strerror("uncompressed message size exceeds the maximum size");
				nt_log(LOG_LEVEL_WARNING, "Uncompressed message size " NT_FS_UI64 " from %s"
						" exceeds the maximum size " NT_FS_UI64 " bytes. Message ignored.",
			...
			else
			{
				...
				{
					nt_set_socket_strerror("cannot allocate memory: out of memory");
					nt_log(LOG_LEVEL_WARNING, "Message size " NT_FS_UI64
			...
			context->expect = NT_TCP_EXPECT_SIZE;
	...
	if (NT_TCP_EXPECT_SIZE == context->expect)
	{
		if (context->buf_stat_bytes + context->buf_dyn_bytes == context->expected_len)
		{
			if (0 != (context->protocol_version & NT_TCP_COMPRESS))
			{
				char	*out;
				size_t	out_size = context->reserved;
				if (NULL == (out = (char *)malloc(context->reserved + 1)))
				{
					nt_set_socket_strerror("cannot allocate memory to uncompress data:"
							" out of memory");
					nt_log(LOG_LEVEL_WARNING, "Uncompressed message size " NT_FS_UI64
							" from %s exceeds the available memory size."
				...
				if (FAIL == nt_uncompress(s->buffer, context->buf_stat_bytes + context->buf_dyn_bytes,
						out, &out_size))
				{
					nt_free(out);
					nt_set_socket_strerror("cannot uncompress data: %s", nt_compress_strerror());
					nbytes = NT_PROTO_ERROR;
					goto out;
				}
				...
				{
					nt_free(out);
					nt_set_socket_strerror("size of uncompressed data is less than expected");
					nbytes = NT_PROTO_ERROR;
					goto out;
				}
				...
				nt_log(LOG_LEVEL_TRACE, "%s(): received " NT_FS_SIZE_T " bytes with"
						" compression ratio %.1f", __func__,
						(nt_fs_size_t)(context->buf_stat_bytes + context->buf_dyn_bytes),
		...
		{
			nt_set_socket_strerror("message length does not match expected length");
			if (context->buf_stat_bytes + context->buf_dyn_bytes < context->expected_len)
			{
				nt_log(LOG_LEVEL_WARNING, "Message from %s is shorter than expected " NT_FS_UI64
						" bytes. Message ignored.", s->peer,
						(nt_uint64_t)context->expected_len);
			}
			else
			{
				nt_log(LOG_LEVEL_WARNING, "Message from %s is longer than expected " NT_FS_UI64
						" bytes. Message ignored.", s->peer,
						(nt_uint64_t)context->expected_len);
			}
	...
	else if (NT_TCP_EXPECT_LENGTH == context->expect)
	{
		nt_set_socket_strerror("message is missing data length");
		nt_log(LOG_LEVEL_WARNING, "Message from %s is missing data length. Message ignored.", s->peer);
		nbytes = NT_PROTO_ERROR;
	}
	else if (NT_TCP_EXPECT_VERSION == context->expect)
	{
		nt_set_socket_strerror("message is missing protocol version");
		nt_log(LOG_LEVEL_WARNING, "Message from %s is missing protocol version. Message ignored.",
				s->peer);
		nbytes = NT_PROTO_ERROR;
	}
	else if (NT_TCP_EXPECT_VERSION_VALIDATE == context->expect)
	{
		nt_set_socket_strerror("message is using unsupported protocol version");
		nt_log(LOG_LEVEL_WARNING, "Message from %s is using unsupported protocol version \"%d\"."
				" Message ignored.", s->peer, context->protocol_version);
		nbytes = NT_PROTO_ERROR;
	}
	else if (0 != context->buf_stat_bytes)
	{
		nt_set_socket_strerror("message is missing header");
		nt_log(LOG_LEVEL_WARNING, "Message from %s is missing header. Message ignored.", s->peer);
		nbytes = NT_PROTO_ERROR;
	}
	else
	{
...
out:
	return (NT_PROTO_ERROR == nbytes ? FAIL : (ssize_t)(s->read_bytes + context->offset));
...
{
	nt_tcp_recv_context_t	tcp_recv_context;
	ssize_t			nbytes;

	if (0 != timeout)
		nt_socket_set_deadline(s, timeout);

	nt_tcp_recv_context_init(s, &tcp_recv_context, flags);

	nbytes = nt_tcp_recv_context(s, &tcp_recv_context, flags, NULL);
...
ssize_t	nt_tcp_recv_raw_ext(nt_socket_t *s, int timeout)
{
	...
	nt_tcp_recv_context_init(s, &tcp_recv_context, 0);

	nbytes = nt_tcp_recv_context_raw(s, &tcp_recv_context, NULL, 0);

	if (0 != timeout)
		nt_socket_set_deadline(s, 0);

	return (NT_PROTO_ERROR == nbytes ? FAIL : nbytes);
}
 ...
 *               NT_PROTO_ERROR - an error occurred                          *
 *                                                                            *
...
ssize_t	nt_tcp_recv_context_raw(nt_socket_t *s, nt_tcp_recv_context_t *context, short *events, int once)
{
			...
			sizeof(s->buf_stat) - context->buf_stat_bytes, events)))
	{
		...
		else
		{
			if (context->buf_dyn_bytes + (size_t)nbytes >= context->allocated)
			{
				...
				s->buffer = (char *)nt_realloc(s->buffer, context->allocated);
			}
		...
		{
			s->buf_type = NT_BUF_TYPE_DYN;
			context->allocated = 8 * NT_STAT_BUF_LEN;
			s->buffer = (char *)nt_malloc(NULL, context->allocated);
	...
	if (context->buf_stat_bytes + context->buf_dyn_bytes >= context->expected_len)
	{
		nt_log(LOG_LEVEL_WARNING, "Message from %s is longer than " NT_FS_UI64 " bytes allowed for"
	...
	s->buffer[s->read_bytes] = '\0';
out:
	return (NT_PROTO_ERROR == nbytes ? NT_PROTO_ERROR : (ssize_t)(s->read_bytes));
}
...
const char	*nt_tcp_recv_context_line(nt_socket_t *s, nt_tcp_recv_context_t *context, short *events)
{
	...
	if (NULL != s->next_line)
	{
		...
		memmove(s->buffer, s->next_line, NT_BUF_TYPE_STAT == s->buf_type ?
	...
	do
	{
		...
		if (NT_PROTO_ERROR == (nbytes = nt_tcp_recv_context_raw(s, context, events, 1)))
			goto out;
...
static int	subnet_match(int af, unsigned int prefix_size, const void *address1, const void *address2)
{
	...
	if (af == AF_INET)
	{
		if (prefix_size > NT_IPV4_MAX_CIDR_PREFIX)
			return FAIL;
		bytes = 4;
	}
	else
	{
		if (prefix_size > NT_IPV6_MAX_CIDR_PREFIX)
			return FAIL;
		bytes = 16;
	}
...
{
	if (SUCCEED == nt_is_ip4(ip))
		return nt_is_uint_range(cidr, value, 0, NT_IPV4_MAX_CIDR_PREFIX);
#ifdef HAVE_IPV6
	if (SUCCEED == nt_is_ip6(ip))
		return nt_is_uint_range(cidr, value, 0, NT_IPV6_MAX_CIDR_PREFIX);
#endif
	return FAIL;
}
...
int	nt_validate_peer_list(const char *peer_list, char **error)
{
	...
	for (start = tmp; '\0' != *start;)
	{
		...
		else if (FAIL == nt_is_supported_ip(start) && FAIL == nt_validate_hostname(start))
		{
			*error = nt_dsprintf(NULL, "\"%s\"", start);
			return FAIL;
		}
			...
			for (current_ai = ai; NULL != current_ai; current_ai = current_ai->ai_next)
			{
				...
				if (-1 == prefix_size_current)
				{
					prefix_size_current = (current_ai->ai_family == AF_INET ?
							NT_IPV4_MAX_CIDR_PREFIX : NT_IPV6_MAX_CIDR_PREFIX);
				}

				if (SUCCEED == nt_ip_cmp((unsigned int)prefix_size_current, current_ai, peer_info, 0))
				{
					freeaddrinfo(ai);
					return SUCCEED;
				}
...
{
	if (SUCCEED == nt_tcp_check_allowed_peers_info(&s->peer_info, peer_list))
		return SUCCEED;
...
int	nt_udp_send(nt_socket_t *s, const char *data, size_t data_len, int timeout)
{
	...
	{
		if (NT_PROTO_ERROR == (n = nt_sendto(s->socket, data + offset, data_len - (size_t)offset, 0, NULL,
				0)))
		{
			int	rc;
			...
			if (SUCCEED != nt_socket_had_nonblocking_error())
			{
				nt_set_socket_strerror("sendto() failed: %s",
						nt_strerror_from_system(nt_socket_last_error()));
				return FAIL;
			}
		...
		if (SUCCEED != nt_socket_check_deadline(s))
		{
			nt_set_socket_strerror("send timeout");
			return FAIL;
		}
	}
...
int	nt_udp_recv(nt_socket_t *s, int timeout)
{
	...
	while (0 >= (n = recvfrom(s->socket, buffer, sizeof(buffer) - 1, 0, NULL, NULL)))
	{
		...
		if (0 == n)
		{
			nt_set_socket_strerror("connection shutdown");
			return FAIL;
		}

		if (SUCCEED != nt_socket_had_nonblocking_error())
		{
			nt_set_socket_strerror("recvfrom() failed: %s",
					nt_strerror_from_system(nt_socket_last_error()));
			return FAIL;
		}
		...
		if (SUCCEED != nt_socket_check_deadline(s))
		{
			nt_set_socket_strerror("recv timeout");
			return FAIL;
		}
		...
		if (0 == (pd.revents & POLLIN))
		{
			...
			nt_log(LOG_LEVEL_DEBUG, "poll(POLLIN) failed with revents 0x%x",
					(unsigned)pd.revents);
	...
	{
		s->buf_type = NT_BUF_TYPE_DYN;
		s->buffer = (char *)nt_malloc(s->buffer, (size_t)n + 1);
	}
